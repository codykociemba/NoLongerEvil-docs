...
<Tip>
  **Why all URLs point to the same server**: For simplicity, No Longer Evil uses a single server for all services. Google's infrastructure uses separate servers for each service.
</Tip>
...
sequenceDiagram
    ...
    participant API as No Longer Evil API
